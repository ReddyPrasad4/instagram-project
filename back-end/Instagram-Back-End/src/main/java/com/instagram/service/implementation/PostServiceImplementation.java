...
public class PostServiceImplementation implements PostService {
    ...
    public ResponseEntity<List<PostDTO>> getAllPosts() {
        ...
        try {
            ...
            if (!CollectionUtils.isEmpty(postList)) {
                List<PostDTO> postDTOList = converToPostDtoList(postList);
    ...
    public List<PostDTO> converToPostDtoList(List<Post> postList)
    {
        return postList.stream().map(post -> {
            ...
            postDTO.setCommentsDTOList(post.getComments().stream().filter(comment -> !comment.getIsDeleted() && !comment.getUser().getIsDeleted()).map(comment -> convertion.convertToDto(comment, CommentDTO.class)).toList());
    ...
    public ResponseEntity<PostDTO> getPostById(UUID postId) {
        PostDTO responsePostDTO = new PostDTO();
        try {
            Post post = postRepository.findByIdAndIsDeleted(postId, Boolean.FALSE);
            ...
            responsePostDTO.setMessage(ApplicationConstants.POST_NOT_FOUND);
            return new ResponseEntity<>(responsePostDTO, HttpStatus.BAD_REQUEST);
        } catch (Exception e) {
            log.error(e.getLocalizedMessage());
            responsePostDTO.setMessage(e.getMessage());
            return new ResponseEntity<>(responsePostDTO, HttpStatus.BAD_REQUEST);
        }
    }
