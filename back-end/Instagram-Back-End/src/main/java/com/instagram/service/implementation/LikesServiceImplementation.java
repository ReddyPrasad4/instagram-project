...
public class LikesServiceImplementation implements LikesService {
        ...
        try {
            if (!ObjectUtils.isEmpty(likesDTO.getPostDto().getId()) && StringUtils.isNotEmpty(userName)) {
                ...
                if (!ObjectUtils.isEmpty(user)) {
                    Likes convertedLikesDTO = convertion.convertToEntity(likesDTO, Likes.class);
                    convertedLikesDTO.setUser(user);
                    likesRepository.save(convertedLikesDTO);
    ...
    @Override
    public ResponseEntity<BaseResponseDTO> updateLike(LikesDTO likesDTO) {
        BaseResponseDTO baseResponseDTO = new BaseResponseDTO();
        try {
            ...
            log.error(e.getLocalizedMessage());
            baseResponseDTO.setMessage(ApplicationConstants.ERROR_UPDATING_LIKE + e.getLocalizedMessage());
            return new ResponseEntity<>(baseResponseDTO, HttpStatus.BAD_REQUEST);
        }
    }
